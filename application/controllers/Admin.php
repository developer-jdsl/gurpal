...
class Admin extends CI_Controller {
   ...
   */	
		...
		public function users()
	{
		...
		$this->load->view('admin/templates/footer');
	}					/*    #######################################   PAGES MODULE    #######################################   */            public function pages()	{		is_superadmin();		$this->load->view('admin/templates/header');		$this->load->view('admin/templates/sidebar');		$this->load->view('admin/templates/topbar');		$this->load->view('admin/pages/pages');		$this->load->view('admin/templates/footer');	}		public function page_list()	{	      $list = $this->admin_model->get_pages();      $data = array();      $no = $this->input->post('start');      foreach ($list as $ro) {      $no++;      $row = array();			 $row[] = $ro->page_name;		 $row[] = $ro->active==1?'Active':'Inactive';			$row[] = $ro->show_on_menu==1?'Yes':'No';			 $row[] = form_open('admin/edit_page',array('class'=>'d-inline')).'				   <input type="hidden" name="id" value="'.$ro->pk_page_id.'"> 				   <button  class="btn btn-primary" type="submit">'.keyword_value('edit','Edit').'</button>				   </form>'.form_open('admin/delete_page',array('class'=>'d-inline')).'				   <input type="hidden" name="id" value="'.$ro->pk_page_id.'"> 				   <button class="btn btn-primary" type="submit">'.keyword_value('delete','Delete').'</button>				   </form>				   <a href="'.base_url($ro->page_slug).'" class="btn btn-primary"><i class="fa fa-eye"></i></a>';      $data[] = $row;      }      $output = array(      "draw" => $this->input->post('draw'),      "recordsTotal" => $this->admin_model->pages_count_all(),      "recordsFiltered" => $this->admin_model->pages_count_filtered(),      "data" => $data,      );      //output to json format      echo json_encode($output);      }	  	  	  public function add_page()	{		is_superadmin();		$this->form_validation->set_rules('page_name', 'Page Name', 'required',			array('required' =>  keyword_value('you_must_enter_page_name','You must Enter Page Name.'))		);					  if ($this->form_validation->run() == FALSE)		{			$this->load->view('admin/templates/header');			$this->load->view('admin/templates/sidebar');			$this->load->view('admin/templates/topbar');			$this->load->view('admin/pages/add_page');			$this->load->view('admin/templates/footer');		}		else		{						$data['page_name']=$this->input->post('page_name');			$data['page_content']=$this->input->post('page_content');			$data['meta_title']			=	$this->input->post('meta_title')?$this->input->post('meta_title'):$this->input->post('page_name');			$data['meta_keywords']		=	$this->input->post('meta_keywords');			$data['meta_description']	=	$this->input->post('meta_description');			$data['ordering']			=	$this->input->post('ordering');			$data['active']				=	$this->input->post('active');			$data['show_on_menu']		=	$this->input->post('show_on_menu');			$return=$this->admin_model->add_page($data);			if($return['status']==true)			{				$this->session->set_flashdata('msg', keyword_value('page_added','Page Added Successfully'));				redirect('admin/pages');						}			else			{								$this->session->set_flashdata('msg', keyword_value('page_not_added','Action was not Successfull, Please try again'));				redirect('admin/add_page');			}		}						}				public function edit_page()	{		is_superadmin();				$id=$this->input->post('id');		if($id)		{			$result=$this->admin_model->check_page_id($id);				if($result['pk_page_id'])		{							$this->data['results']=$result;				$this->load->view('admin/templates/header');				$this->load->view('admin/templates/sidebar');				$this->load->view('admin/templates/topbar');				$this->load->view('admin/pages/edit_page',$this->data);				$this->load->view('admin/templates/footer');						}				else				{					$this->session->set_flashdata('msg', keyword_value('invalid_action','Invalid Action'));					redirect('admin/pages');				}				}		else		{					$this->session->set_flashdata('msg', keyword_value('invalid_action','Invalid Action'));			redirect('admin/pages');		}	}				public  function update_page()	{				$id=$this->input->post('id');		if($id)		{			is_superadmin();		$this->form_validation->set_rules('page_name', 'Page Name', 'required',			array('required' =>  keyword_value('you_must_enter_page_name','You must Enter Page Name.'))		);					 if ($this->form_validation->run() == FALSE)		{			$this->load->view('admin/templates/header');		$this->load->view('admin/templates/sidebar');		$this->load->view('admin/templates/topbar');		$this->load->view('admin/pages/edit_page',$this->data);		$this->load->view('admin/templates/footer');		}		else		{		   					$data['page_slug']			=	$this->input->post('page_slug');			$data['page_name']			=	$this->input->post('page_name');			$data['page_content']		=	$this->input->post('page_content');			$data['meta_title']			=	$this->input->post('meta_title')?$this->input->post('meta_title'):$this->input->post('page_name');			$data['meta_keywords']		=	$this->input->post('meta_keywords');			$data['meta_description']	=	$this->input->post('meta_description');			$data['ordering']			=	$this->input->post('ordering');			$data['active']				=	$this->input->post('active');			$data['show_on_menu']		=	$this->input->post('show_on_menu');						$return=$this->admin_model->edit_page($data,$id);					if($return['status']==true)			{				$this->session->set_flashdata('msg', keyword_value('page_updated','Page Updated Successfully'));				redirect('admin/pages');			}			else			{								$this->session->set_flashdata('msg', keyword_value('item_not_added','Action was not Successfull, Please try again'));				redirect('admin/pages');			}								}				}		else		{					$this->session->set_flashdata('msg', keyword_value('invalid_action','Invalid Action'));			redirect('admin/pages');		}	}	public function delete_page()	{				$id=$this->input->post('id');		if($id)		{			$result=$this->admin_model->check_page_id($id);				if($result['pk_page_id'])								{								$this->data['results']=$result;				$this->load->view('admin/templates/header');				$this->load->view('admin/templates/sidebar');				$this->load->view('admin/templates/topbar');				$this->load->view('admin/pages/delete_page',$this->data);				$this->load->view('admin/templates/footer');						}				else				{					$this->session->set_flashdata('msg', keyword_value('invalid_action','Invalid Action'));					redirect('admin/pages');				}				}		else		{					$this->session->set_flashdata('msg', keyword_value('invalid_action','Invalid Action'));			redirect('admin/pages');		}					}		public function remove_page()	{		$id=$this->input->post('id');		if($id)		{						$return=$this->admin_model->remove_page($id);									if($return['status']==true)			{				$this->session->set_flashdata('msg', keyword_value('item_deleted','Item Deleted Successfully'));				redirect('admin/pages');			}			else			{								$this->session->set_flashdata('msg', keyword_value('item_not_added','Action was not Successfull, Please try again'));				redirect('admin/pages');			}		}				else		{			$this->session->set_flashdata('msg', keyword_value('invalid_action','Invalid Action'));			redirect('admin/pages');		}			}  			public function upload_image()	{				if($_FILES['file']["name"])			{			  $config['upload_path']   = './uploads/pages/'; 			  $config['allowed_types'] = 'gif|jpg|png|jpeg'; 			  $config['max_size']      = 1024;			  $this->load->library('upload', $config);				   if ( ! $this->upload->do_upload('file')) {			 $error = implode('<br>',$this->upload->display_errors()); 				  }else { 			$uploadedImage = $this->upload->data();			$image=$uploadedImage['file_name'];						echo json_encode(array('file_path' => base_url('uploads/pages/'.$image)));      } 	  	    			}			}		/* Reports blank module */				 public function reports()	{		$this->load->view('admin/templates/header');		$this->load->view('admin/templates/sidebar');		$this->load->view('admin/templates/topbar');		$this->load->view('admin/reports/reports');		$this->load->view('admin/templates/footer');	}
